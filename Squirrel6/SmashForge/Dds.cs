    ...
    public class Dds {
        ...
        public void FromNutTexture(NutTexture tex) {
            ...
            switch (tex.pixelInternalFormat) {
                ...
                case PixelInternalFormat.Rgb32i:
                    header.ddspf.fourCc = 0x00000000;
                    if (tex.pixelFormat == OpenTK.Graphics.OpenGL.PixelFormat.Rgb) {
                        header.ddspf.flags = (uint)(Ddpf.Rgb);
                        header.ddspf.rgbBitCount = 0x8 * 3;
                        header.ddspf.rBitMask = 0x000000FF;
                        header.ddspf.gBitMask = 0x0000FF00;
                        header.ddspf.bBitMask = 0x00FF0000;
                    }
        ...
        public Bitmap ToBitmap() {
            ...
            } else if (header.ddspf.fourCc == 0x0) {
                DecodeBgra(pixels, bdata, (int)header.width, (int)header.height);
            } else {
                Program.WriteLineIfNotQuiet("Unknown DDS format " + header.ddspf.fourCc.ToString("X"));
            }
        ...
        public static void DecodeBgra(byte[] pixels, byte[] data, int width, int height) {
            Program.WriteLineIfVerbose("DecodeBgra " + pixels.Length + " <- " + data.Length);
            for (int x = 0; x < data.Length && x < pixels.Length; x += 4) {
                ...
                Array.Copy(newPixel, 0, pixels, x, 4);
            }
        }
